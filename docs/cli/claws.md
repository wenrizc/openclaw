...
prerequisite, blocker, capability change, and a `planIntegrity` digest. Review
the plan before creating the agent:
...
including blockers and a `planIntegrity` digest. Capability escalations have
separate machine-readable records and `!` lines in human output. The exact
integrity confirmation binds that disclosed capability set as well as ordinary
